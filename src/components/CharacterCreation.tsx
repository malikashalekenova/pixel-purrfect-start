...
export function CharacterCreation({ onCreated }: Props) {
  ...
  const handleAuthSubmit = async (e: FormEvent) => {
    ...
    if (!isSupabaseConfigured()) {
      setErr("Сохранение временно недоступно. Можно играть как гость.");
      return;
    }
    ...
    try {
      ...
      if (!signUpData.session) {
        setErr("Проверь почту и подтверди email, затем нажми «Войти».");
        setMode("login");
        return;
      }

      // Ensure profile exists (trigger may have failed silently).
      const profile = await withTimeout(ensureCurrentProfile(), 8000, "Загрузка профиля");
      setExistingProfile(profile);
  ...
  const handleGoogle = async () => {
    ...
    setErr(null);
    if (!isSupabaseConfigured()) {
      setErr("Google вход временно недоступен. Можно играть как гость.");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-stretch justify-center bg-[#0a0f1a] overflow-y-auto animate-fade-in">
      ...
      >
        <div className="mb-6 text-center">
          <p className="text-[10px] uppercase tracking-[0.3em] text-cyan-300/80">Шаг 2 из 2</p>
          <h2 className="mt-2 text-2xl sm:text-3xl font-bold">Создание персонажа</h2>
          <p className="mt-2 text-xs text-white/50">Расскажи о себе, чтобы город тебя запомнил.</p>
        </div>
